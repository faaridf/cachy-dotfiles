...
local function clipshot(arg)
	return function()
		-- 1. Cache your original global settings from mpv.conf
		local old_depth = mp.get_property("screenshot-high-bit-depth")
		local old_comp = mp.get_property("screenshot-png-compression")

		-- 2. Force zero compression for the temporary clipboard file
		mp.set_property("screenshot-high-bit-depth", "no")
		mp.set_property("screenshot-png-compression", "0")

		-- 3. Take the screenshot (this runs synchronously)
		mp.commandv("screenshot-to-file", file, arg)

		-- 4. Immediately restore your original global configurations
		mp.set_property("screenshot-high-bit-depth", old_depth)
		mp.set_property("screenshot-png-compression", old_comp)

		-- 5. Pass the temporary file payload to your clipboard manager
