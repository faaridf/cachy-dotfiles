...
local options = {
	...
	output_template = "%{media-title}-[%s-%e]%M",
...
expand_properties = function(text, magic)
	...
	do
		...
		if colon == ":" then
			...
			local _
			text, _ = text:gsub(
				"%" .. magic .. "{" .. prefix .. raw .. original_prop:gsub("%W", "%%%1") .. closing .. "}",
				prop_value
			)
		end
	end
...
format_filename = function(startTime, endTime, videoFormat)
	...
	local replaceTable = {
		...
		["%%M"] = (mp.get_property_native("aid") and not mp.get_property_native("mute") and hasAudioCodec) and "-audio"
			or "",
		["%%R"] = (options.scale_height ~= -1) and "-" .. tostring(options.scale_height) .. "p"
			or "-" .. tostring(mp.get_property_native("height")) .. "p",
...
do
	...
	local _base_0 = {
		...
		draw = function(self)
			...
			ass:append(
				tostring(bold("ENTER:")) .. " confirm crop (" .. tostring(width) .. "x" .. tostring(height) .. ")\\N"
			)
